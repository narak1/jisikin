...
bool find_item_info(int item_id, char item_name[], int& item_price);
...
// item info print
	...
	if( flag == false ) {
		cout << "Item Record is not found... try again" << endl << endl << endl;
		return;
	}
...
// sale info print
	...
	cout << "\n\t\t" << buyer_name << "\t\t" << item_name << "\t" << amount << ".00" << endl;
	...
	ofs << setw(20) << buyer_name << setw(20) << item_name << setw(20) << amount << endl;
	ofs.close();
	cout << "\nRecord saved..." << endl << endl;
}
...
bool find_item_info(int item_id, char item_name[], int& item_price)
{
	...
	bool flag = false;
	while( ! ifs.eof() ) {
		ifs >> cur_item_id >> item_name >> item_price;
...
void display_sales()
{
	...
	while( ! ifs.eof() ) {
		ifs >> buyer_name;
		if( ifs.eof() ) break;
		ifs >> item_name >> amount;
		cout << setw(20) << buyer_name << setw(20) << item_name << setw(20) << amount << endl;
	}
	cout << endl;
