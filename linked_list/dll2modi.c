...
DLN* search(DLN* head, char* buff)
{
	...
	p = head->rlink;
	while(p!=head){
		if(strcmp(buff,p->start_node) == 0) return p;
		p=p->rlink;
	}
	return NULL;
}
...
int time_account(DLN* s, DLN* e)
{
	...
	while(p != e){
		...
		printf("p: start_node=%s, time=%d, dist=%lf\n", p->start_node, p->time, p->distance );
...
main()
{
	...
	DLN* p, *pre; //최대 NUM개 입력가능으로 한정짓는다.
	int cnt;
	char buffer[20];
	DLN* start_node;
	DLN* end_node;
	...
	{
		p = (DLN*)malloc(sizeof(DLN));
		printf("%d번째 목적지 데이터를 입력하시오....\n",cnt+1);
		printf("현재 출발지 이름(영문) : "); gets(p->start_node); fflush(stdin);
		printf("다음 도착지 이름(영문) : "); gets(p->next_node); fflush(stdin);
		printf("다음 도착지까지의 시간(단위:분) : "); scanf("%d",&(p->time)); fflush(stdin);
		printf("다음 도착지까지의 거리(단위:KM) : "); scanf("%lf", &(p->distance)); fflush(stdin);
		dinsert_node(pre, p);		
		pre = p;
	}
